...
{if isset($tc_config.YBC_TC_LAYOUT) && $tc_config.YBC_TC_LAYOUT|escape:'html':'UTF-8' == 'LayoutHome1'}
          ...
          <ul class="{if isset($depth) && $depth == 0}category-top-menu-list {/if}category-sub-menu" data-show="{$depth|escape:'html':'UTF-8'}">
            ...
            {foreach from=$nodes item=node}
              ...
              <li {if isset($i) && $i > 10} class="hidden_product"{/if}>
                    ...
                    {if $depth===0}
                      <a href="{$node.link|escape:'quotes':'UTF-8'}">{$node.name|escape:'html':'UTF-8'}</a>
                      {if $node.children}
                        <div class="navbar-toggler collapse-icons" data-toggle="collapse" data-target="#customexCollapsingNavbar{$node.id|intval}">
                          <i class="ion-ios-arrow-right add"></i>
                        </div>
                        <div class="collapse sub_cat_hover" id="customexCollapsingNavbar{$node.id|intval}">
                          {categories_custom nodes=$node.children depth=$depth+1}
                        </div>
                      {/if}
                    {else}
                      <a class="category-sub-link" href="{$node.link|escape:'quotes':'UTF-8'}">{$node.name|escape:'html':'UTF-8'}</a>
                      {if $node.children}
                        <span class="arrows" data-toggle="collapse" data-target="#customexCollapsingNavbar{$node.id|intval}">
