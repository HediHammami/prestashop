...
pc_ets_reduction[{$combination.id_product_attribute|intval}] = '{if isset($combination.specific_prices.reduction_type) && $combination.specific_prices.reduction_type}-{($combination.specific_prices.reduction * 100)|floatval}%{else}0{/if}';
{/foreach}
</script>
{/if}
{if isset($configs.ETS_PT_HOOK_TO) && $configs.ETS_PT_HOOK_TO|escape:'html':'UTF-8' == 'displayFooterProduct'}
    <div id="ets_purchasetogether" class="ets_purchase_footerproduct">
{elseif isset($configs.ETS_PT_HOOK_TO) && $configs.ETS_PT_HOOK_TO == 'displayProductAdditionalInfo'}
    <div id="ets_purchasetogether" class="ets_purchase_productaddition">
{else}
    <div id="ets_purchasetogether">
{/if}
{if isset($configs.ETS_PT_DISPLAY_TYPE) && $configs.ETS_PT_DISPLAY_TYPE|intval == 1}
    {if isset($purchase_togethers) && $purchase_togethers && count($purchase_togethers) != $alldisabled}
        ...
        <ul class="ets-list-purchase-together ets-list-purchase-type-grid">
            {foreach from=$purchase_togethers item=product}
                {if isset($product.disabled) && !$product.disabled|intval}
                    <li class="ajax_block_product actived" data-id-product="{$product.id_product|intval}" data-id-product-attribute="{$product.id_product_attribute|intval}">
                        ...
                        <div class="price">
                			{if !isset($restricted_country_mode) &&isset($configs.ETS_PT_DISPLAY_PRODUCT_PRICE) && $configs.ETS_PT_DISPLAY_PRODUCT_PRICE}
        						...
        						{if $currProduct.price_without_reduction > 0 &&isset($currProduct.specific_prices) && $currProduct.specific_prices &&isset($currProduct.specific_prices.reduction) && $currProduct.specific_prices.reduction > 0}
        							{if isset($configs.ETS_PT_DISPLAY_OLD_PRICE) && $configs.ETS_PT_DISPLAY_OLD_PRICE|intval}
                            		...
                            		{if $currProduct.specific_prices.reduction_type == 'percentage' &&isset($configs.ETS_PT_DISPLAY_DISCOUNT) && $configs.ETS_PT_DISPLAY_DISCOUNT}
        								<span class="price-percent-reduction">-{($currProduct.specific_prices.reduction * 100)|floatval}%</span>
        							{/if}
        						{/if}
        					{/if}
                        </div>
                        <div class="description">
                            {if isset($configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION) && $configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION|intval}
                                <div  class="">{if isset($currProduct.description_short) && $currProduct.description_short|escape:'html':'UTF-8'}{$currProduct.description_short|escape:'html':'UTF-8'|truncate:$configs.ETS_PT_MAX_DESCRIPTION_LENGHT:'...'}
                                    {else}{$currProduct.description|escape:'html':'UTF-8'|truncate:$configs.ETS_PT_MAX_DESCRIPTION_LENGHT:'...'}{/if}</div>
                            {/if} 
                        </div>
                        {if isset($configs.ETS_PT_DISPLAY_RATING) && $configs.ETS_PT_DISPLAY_RATING|intval}
                        ...
                        <div class="row-product">
                            <div class="ets_purchase_item_image">
                                <input type="checkbox" class=""
                                    ...
                                    data-qty="{if isset($configs.ETS_PT_DEFAULT_QUANTITY_ADDED_TO_CART) && $configs.ETS_PT_DEFAULT_QUANTITY_ADDED_TO_CART}{$configs.ETS_PT_DEFAULT_QUANTITY_ADDED_TO_CART|intval}{else}1{/if}"
                            ...
                            <div class="ets_purchase_item_des">
                                ...
                                <span class="price">
                        			{if isset($product.show_price) && $product.show_price|intval && !isset($restricted_country_mode) &&isset($configs.ETS_PT_DISPLAY_PRODUCT_PRICE) && $configs.ETS_PT_DISPLAY_PRODUCT_PRICE|intval}
            							<span itemprop="price" class="price product-price">
                                ...
                                <div class="description">
                                    {if isset($configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION) && $configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION|intval}
                                        <div  class="">{if isset($product.description_short|escape:'html':'UTF-8') && $product.description_short}{$product.description_short|truncate:$configs.ETS_PT_MAX_DESCRIPTION_LENGHT:'...' nofilter}
    	...
    	<div class="button-container {if count($purchase_togethers) == $alldisabled}disabled{/if}">
    		<a class="button ets_ajax_add_to_cart_button btn btn-default" href="{$ajax_cart|escape:'quotes':'UTF-8'}" rel="nofollow" title="{l s='Add all products to cart' mod='ets_purchasetogether'}">
            ...
            {if (isset($configs.ETS_PT_EXCLUDE_CURRENT_PRODUCT) && !$configs.ETS_PT_EXCLUDE_CURRENT_PRODUCT)
                ...
                <li class="item-product this-product">
                    ...
                    <div class="ets_purchase_item_des">                    
                        ...
                        <div class="description">
                            {if isset($configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION) && $configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION|intval}
                                <div  class="">{if isset($currProduct.description_short) && $currProduct.description_short|escape:'html':'UTF-8'}{$currProduct.description_short|truncate:$configs.ETS_PT_MAX_DESCRIPTION_LENGHT:'...'}
                            ...
                            <div class="ets_purchase_item_des">
                                ...
                                <div class="description">
                                    {if isset($configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION) && $configs.ETS_PT_DISPLAY_PRODUCT_DESCRIPTION}
                                        <div  class="">{if isset($product.description_short) && $product.description_short|escape:'html':'UTF-8'}{$product.description_short|truncate:$configs.ETS_PT_MAX_DESCRIPTION_LENGHT:'...' nofilter}
                                            {else}{$product.description|truncate:$configs.ETS_PT_MAX_DESCRIPTION_LENGHT:'...' nofilter}{/if}</div>
                                    {/if} 
                                </div>
                                {if isset($configs.ETS_PT_DISPLAY_RATING) && $configs.ETS_PT_DISPLAY_RATING|intval}
...
{elseif isset($configs.ETS_PT_HOOK_TO) && $configs.ETS_PT_HOOK_TO|escape:'html':'UTF-8' == 'displayProductAdditionalInfo'}
    </div>
{else}
    </div>
{/if}
