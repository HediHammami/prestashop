...
<div id="product-purchasetogether" class="{if isset($is17) && !$is17}panel product-tab{/if}" data-url-ajax="{$url_ajax|escape:'html':'UTF-8'}">
    ...
    <div class="form-group">
		...
		<div class="col-lg-9">
			<input type="hidden" name="inputPurchaseTogether" id="inputPurchaseTogether" value="{if $purchase_togethers}{foreach from=$purchase_togethers item=purchase}{$purchase.id_product|intval}-{if $purchase.id_product_attribute}{$purchase.id_product_attribute|intval}{else}0{/if},{/foreach}{/if}" />
			...
			{if $purchase_togethers}
				...
				<div class="form-control-static">
                    ...
                    {if $purchase.image}<img src="{$purchase.image|escape:'quotes':'UTF-8'}" alt="{$purchase.name|escape:'html':'UTF-8'}" width="80px" height="auto" />{/if}
