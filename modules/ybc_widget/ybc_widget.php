...
class Ybc_widget extends Module
{
	...
	{	   
        ...
        $this->_hooks = array(
            ...
            array(
                'hook' => 'ybcCustom1',
                'name' => $this->l('Widget custom')
            ),
            array(
                'hook' => 'ybcCustom2',
                'name' => $this->l('Home top banner')
            ), 
            array(
                'hook' => 'ybcCustom3',
                'name' => $this->l('banner long in homepage')
            ),
            array(
                'hook' => 'ybcCustom4',
            ...
            array(
                'hook' => 'ybcCustom5',
                'name' => $this->l('Tesimonial on home')
            ), 
            array(
                'hook' => 'ybcCustom6',
                'name' => $this->l('Home top colum layout 3')
            ),
            array(
                'hook' => 'ybcCustom7',
                'name' => $this->l('Home bottom colum layout 3')
            ),              
        );
    ...
    public function getMaxId($tbl, $primaryKey)
    {
        $req = 'SELECT max(`'.$primaryKey.'`) as maxid
				FROM `'._DB_PREFIX_.'ybc_widget_'.$tbl.'` tbl';				
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);
        returnisset($row['maxid']) ? (int)$row['maxid'] : 0;
    }
    ...
    public function hookYbcCustom7()
    {
        $widgets = $this->getWidgetsByHook('ybcCustom7', true);
        $this->smarty->assign(
            array(
                'widgets' => $widgets,
                'widget_module_path' => $this->_path,
                'widget_hook' => 'ybc-custom-7'
            )
        );
        return $this->display(__FILE__, 'widgets.tpl');
    }
